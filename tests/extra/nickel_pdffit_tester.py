...
def GofR(rk,r,sig,Np,c,b,bavg,rho=1.): 
    ...
    for r_ij, sig_ij, c_ij, b_ij, bi_avg in zip(r,sig, c, b, bavg): 
        ...
        GofR += g_ij 
    #return (1./(Np*rk)) *GofR - (4.*np.pi * rho * rk )
...
class nickel(object):
    ...
    def __init__(self):
        ...
        self._avg_scat_lengths = np.full_like(self._distances, 
                                              np.sum(self._b*self._concentrations/self._Np))
    ...
    def make_from_ase(self,supercell):
        ...
        if len(supercell) != 3:
            raise Exception("in make_from_ase, supercell must be a vector [a b c]")
        ni_supercell = make_supercell(ni,supercell)
