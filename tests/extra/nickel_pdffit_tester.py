...
def gaussian(x, mu, sig, a=1.0):
    factor = a / (sig * np.sqrt(2.0 * np.pi))
    return factor * np.exp(-np.power(x - mu, 2.0) / (2 * np.power(sig, 2.0)))


def Aij(c, b, bavg):
    return c * b / np.power(bavg, 2.0)
...
class Nickel(object):
    ...
    def __init__(self):
        ...
        self._concentrations = np.full_like(self._distances, 1.0 / self._Np)
    ...
    def make_from_ase(self, supercell):
        ...
        a = self._a
        ni = crystal(self._name, [(0, 0, 0)], spacegroup=self._space_group, cellpar=[a, a, a, 90, 90, 90])
